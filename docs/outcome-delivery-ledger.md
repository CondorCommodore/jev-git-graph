# Jev Git Graph: implementation through preservation and cleanup

This delivery retains the complete operator goal: a useful preservation decision
for every inventoried object, measured Jev utility, verified integration outcomes,
and an approved real Home Lab cleanup batch with restoration evidence.

## Current evidence and work ownership

The baseline is main `8496709`. The private Home Lab pilot recorded 1,449
branches, 622 worktrees and two stashes. Its 1,432 eligible branches produced
14,175 changed paths and 113,211 units. Independent Git comparisons accounted
...
| Creator participation, durable cleanup journal, restore and atomic deletion | Cleanup lane + parent | Creator races and interrupted execution in disposable repo | CLI passes a durable journal and concrete verified adapter. Latest installed-wheel fixture at source head `b1d7335` passed interrupted reconciliation and post-delete drift/no-overwrite checks (wheel SHA-256 `fbe5d387afb970626317821e74e529a59e2ab9a5dca5bd6f0542cf9a3d942067`). Production creator-participation proof remains absent in the live Home Lab checkout; live gate remains closed. |
...
boundaries. Existing required checks remain landing gates. A synthetic result
is labeled simulation and cannot satisfy the real model utility or real cleanup
requirements. Review receipts, test results, provider dispatch, merged code and
actual deleted refs each require their own direct evidence.

## Approval boundaries

The build is authorized. New Jev evidence requires an exact prepared request
manifest and approval. Real cleanup requires its concrete verified recovery
bundle and exact manifest approval. Neither boundary blocks independent build
work. A model answer cannot authorize deletion. Non-EXACT archive-backed deletion,
worktree removal and stash dropping require their own specified preservation
policy and explicit approval if they become necessary.
